...
from rdkit.Chem import rdFingerprintGenerator, Crippen
...
class Evaluator(ABC):
    @abstractmethod
    def evaluate(self, mol) -> np.ndarray:
        pass
...
class LogPandFPEvaluator(Evaluator):
    """An evaluator that computes Crippen LogP and a fingerprint Tanimoto to a reference molecule for an RDKit molecule."""
    ...
    def __init__(self, input_dict):
        ...
        # Setup Crippen LogP
        self.opt_logp = input_dict["opt_logp"]
        self.dev_logp = input_dict["dev_logp"]
    ...
    def evaluate(self, mol):
        ...
        # LogP score calculation
        logp = float(Crippen.MolLogP(mol))
        # Calculate Gaussian score [0,1] based on deviation from optimal logP
        logp_score = np.exp(-0.5 * ((logp - self.opt_logp) / self.dev_logp) ** 2)

        # Tanimoto score calculation
        rd_mol_fp = self.fpgen.GetFingerprint(mol)
        tanimoto_score = DataStructs.TanimotoSimilarity(self.ref_fp, rd_mol_fp)

        return np.array([logp_score, tanimoto_score])


class LogPEvaluator(Evaluator):
    """An evaluator that computes Crippen LogP for an RDKit molecule."""

    def __init__(self, input_dict):
        self.num_evaluations = 0
        self.num_props = 1

        # Get optimal LogP value and deviation from input dictionary
        self.opt_logp = input_dict["opt_logp"]
        self.dev_logp = input_dict["dev_logp"]
...
class FPEvaluator(Evaluator):
    ...
    def evaluate(self, rd_mol_in):
        ...
        return np.array([DataStructs.TanimotoSimilarity(self.ref_fp, rd_mol_fp)])
